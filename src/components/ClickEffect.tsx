 ...
 * caramel burst at the exact cursor point: a soft glow pop, an expanding
 * ring, and a fan of sparkle particles flung outward.
 *
...
export default function ClickEffect() {
  ...
  useEffect(() => {
    function onPointerDown(e: PointerEvent) {
      if (e.button !== 0) return; // primary (left) click / tap only
...
function Burst({ x, y }: { x: number; y: number }) {
  return (
    <>
      {/* Soft glow pop */}
      <span
        className="absolute block rounded-full bg-caramel-400/50 blur-md"
        style={{
          left: x,
          top: y,
          width: 46,
      ...
      {/* Expanding ring */}
      <span
        className="absolute block rounded-full border-2 border-caramel-400"
        style={{
          left: x,
          top: y,
          width: 56,
        ...
        return (
          <span
            ...
            className={
              ...
              (i % 2 ? "bg-caramel-500" : "bg-caramel-400")
            }
            style={
              {
                left: x,
